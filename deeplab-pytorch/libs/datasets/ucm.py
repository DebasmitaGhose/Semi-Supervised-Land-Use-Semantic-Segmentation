...
class UCM(_BaseDataset):
    """
    UC Merced Segmentation dataset
    """

    def __init__(self, **kwargs):
        super(UCM, self).__init__(**kwargs)

    def _set_files(self):
        self.image_dir = osp.join(self.root, "Satellite_Images/UCMerced_Images/") # changed dataset here
        self.label_dir = osp.join(self.root, "Satellite_Images/UCMerced_Labels/") # changed label here

        if self.split in ["all_images", "train", "test"]: # added subset here
            file_list = osp.join(self.root,'Satellite_Images/ImageSets' , self.split + ".txt")
    ...
    def _load_data(self, index):
        ...
        image_path = osp.join(self.root, self.image_dir, image_id + ".tif")
