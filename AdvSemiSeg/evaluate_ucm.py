...
DATA_LIST_PATH = '../../Satellite_Images/ImageSets/test.txt' 
IGNORE_LABEL = 255
NUM_CLASSES = 18
...
def get_iou(data_list, class_num, save_path=None):
    ...
    if save_path:
        with open(save_path, 'w') as f:
            for i, iou in enumerate(j_list):
                f.write('egmentation network to predict feature maps which have the features statistics same as the ground truth, so qualitatively resemble the ground truth. This loss is used on both the segmentation network to predict feature maps which have the features statistics same as the ground truth, so qualitatively resemble the ground truth. This loss is used on both lass {:2d} {:12} IU {:.2f}'.format(i, classes[i], j_list[i]) + '\n')
...
def main():
    ...
    for index, batch in enumerate(testloader):
        ...
        color_file = UCMColorize(output, filename)
